...
class _BatchStockSheetState extends ConsumerState<_BatchStockSheet> {
  ...
  Future<void> _submit() async {
    ...
    // We default to current user if no adjuster is selected, but the UI should require it.
    final currentUser = ref.read(currentProfileProvider);
    final adjusterId = _selectedAdjuster?.id ?? currentUser?.id;
    ...
    try {
      ...
      await repo.batchAdjustStockForGroup(
        ...
        adjustmentType: 'batch_group_adjustment',
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            width: double.infinity,
            child: FilledButton(
              onPressed: (_selectedAdjuster == null || _isLoading) ? null : _submit,
              child: _isLoading 
                ? const SizedBox(
                    height: 20, 
                    width: 20, 
