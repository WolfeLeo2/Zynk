  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        ...List.generate(
          variants.length,
          (i) => _buildVariantRow(variants[i], i, theme, cs),
        ),
        const SizedBox(height: 12),
        // Quick fill banner
        Container(
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(flex: 2, child: _compactField(variant.skuController, 'SKU')),
          const SizedBox(width: 4),
          Expanded(
            flex: 2,
            child: _compactField(
              variant.costController,
              '0.00',
              isNumeric: true,
            ),
          ),
  ...
  DecorationImage? _getImageProvider(VariantRowData variant) {
    if (variant.imageFile != null) {
      return DecorationImage(
        image: FileImage(variant.imageFile!),
        fit: BoxFit.cover,
      );
      ...
      return DecorationImage(
        image: NetworkImage(variant.existingImageUrl!),
        fit: BoxFit.cover,
      );
    }
    return null;
